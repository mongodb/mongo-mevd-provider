...
- Examine the remaining non-flag tokens in priority order: first try external PR mode, then external clone mode, then local range mode.
  ...
  **External PR mode** — if the first non-flag token looks like a PR number (a bare integer such as `12`, or a `#`-prefixed integer such as `#12`), treat this as an external PR review:
  1. Run `gh pr view <PR#> --json number,title,url,baseRefName,headRefOid` and capture the JSON.
  2. Parse `<owner>/<repo>` from the PR URL (e.g. `https://github.com/mongodb/mongo-mevd-provider/pull/12` → `mongodb/mongo-mevd-provider`).
...
- `/review-mevd-provider` → `main...HEAD` in the current repo
- `/review-mevd-provider 12` → external PR #12 (fetches and diffs against its base branch)
- `/review-mevd-provider #12` → same as above
- `/review-mevd-provider HEAD~3` → `HEAD~3...HEAD` in the current repo (last 3 commits only)
- `/review-mevd-provider abc123 def456` → `abc123...def456` in the current repo (arbitrary commit range)
- `/review-mevd-provider --all HEAD~5 HEAD~2` → all reviewers, commits HEAD~5 through HEAD~2 in the current repo
- `/review-mevd-provider ~/code/mevd-pr` → review the current branch of the clone at `~/code/mevd-pr` against its `main`
- `/review-mevd-provider ~/code/mevd-pr release/1.0` → review that clone's `HEAD` against its `release/1.0`
- `/review-mevd-provider ~/code/mevd-pr origin/main feature-x` → review `feature-x` in that clone against `origin/main`
- `/review-mevd-provider --iterate ~/code/mevd-pr` → review the clone's branch, then loop review → fix → review until two consecutive clean passes (or 10 iterations)
- `/review-mevd-provider --iterate --max-iterations 5 ~/code/mevd-pr` → same but cap at 5 iterations
- `/review-mevd-provider --iterate` → iterate on the current repo's branch against `main`
...
|---|---|
| `collection-reviewer` | `src/MongoDB/MongoCollection.cs`, `src/MongoDB/MongoDynamicCollection.cs`, `src/MongoDB/MongoVectorStore.cs`, `src/MongoDB/MongoCollectionCreateMapping.cs`, `src/MongoDB/MongoCollectionTestHook.cs`, `src/MongoDB/MongoConstants.cs`, `src/MongoDB/VectorStoreErrorHandler.cs`, `src/MongoDB/ErrorHandlingAsyncCursor.cs`, `src/MongoDB/Throw.cs` |
| `mapping-reviewer` | `src/MongoDB/MongoMapper.cs`, `src/MongoDB/MongoDynamicMapper.cs`, `src/MongoDB/MongoModelBuilder.cs`, `src/MongoDB/BsonValueFactory.cs`, `src/MongoDB/IMongoMapper.cs` |
| `search-reviewer` | `src/MongoDB/MongoFilterTranslator.cs`, `src/MongoDB/MongoCollectionSearchMapping.cs` |
| `public-api-reviewer` | `src/MongoDB/MongoServiceCollectionExtensions.cs`, `src/MongoDB/MongoVectorStoreOptions.cs`, `src/MongoDB/MongoCollectionOptions.cs` |
| `conformance-reviewer` | `test/MongoDB.ConformanceTests/**` |

**Test-folder mapping.** Per-area test files don't exist as a structured mirror in this repo (the test split is between `test/MongoDB.UnitTests/` for fast no-DB tests and `test/MongoDB.ConformanceTests/` for end-to-end). Apply these rules:

- `test/MongoDB.UnitTests/MongoMapperTests.cs` / `MongoDynamicMapperTests.cs` / `MongoHotelModel.cs` → `mapping-reviewer`.
- `test/MongoDB.UnitTests/MongoCollectionTests.cs` / `MongoVectorStoreTests.cs` → `collection-reviewer`.
- Any other file under `test/MongoDB.UnitTests/` → match by name to the closest source area; route to that reviewer.
- Anything under `test/MongoDB.ConformanceTests/` → `conformance-reviewer`. If the test file is exercising a specific source area (e.g. `MongoFilterTests.cs` exercises filter translation), also route to that source-area reviewer (`search-reviewer` in this example) — the conformance reviewer owns fixture discipline; the area reviewer owns assertion logic.

**Meta-mapping for reviewer definitions.** A change to `.claude/agents/<name>-reviewer.md` is reviewed by that same reviewer (e.g. `.claude/agents/collection-reviewer.md` → `collection-reviewer`). The reviewer is best placed to judge whether its own brief still accurately characterizes the area. Cross-cutting reviewer definitions map to themselves the same way. Exception: `pr-summary-reviewer` only runs in external PR mode, so a local-range or external-clone-mode change to `.claude/agents/pr-summary-reviewer.md` won't be self-reviewed — that file's review happens when the PR is run through `/review-mevd-provider <PR#>`.

If new area reviewers are added under `.claude/agents/`, update this table.

## Cross-cutting reviewers (always run)

These three reviewers run on every invocation of `/review-mevd-provider`, regardless of which files changed. They look across the whole diff for one specific concern. They are *additional to* — not part of — the path-mapping table above.

| Reviewer | Concern |
|---|---|
| `api-stability-reviewer` | Public-API / breaking changes (signatures, defaults, visibility, behavior on unchanged signatures, persisted-document-shape, option-defaults, telemetry attribution like `LibraryInfo` / `ApplicationName`) |
| `mevd-conformance-reviewer` | MEVD abstraction compliance (`VectorStore` / `VectorStoreCollection<TKey, TRecord>` / `IKeywordHybridSearchable<TRecord>` override shape, MEVD `ProviderServices` types, `RecordRetrievalOptions` / `VectorSearchOptions<TRecord>` / `HybridSearchOptions<TRecord>` usage); multi-target hygiene (`net10`/`net8`/`netstandard2.1`/`net472`, AOT/trim attributes); driver-version compatibility |
| `security-reviewer` | Credential exposure in connection strings, log statements, exception messages; TLS bypass surfaces; hardcoded secrets in source / tests / fixtures |
...
- escalate = blocking concern that needs user attention before merge (public-API break, persisted-document-shape change, multi-target build break, MEVD-abstraction shape regression, security regression)
...
- `[fix-in-code]` — the finding can be resolved by an in-tree code change (edit a file, add a test, fix a typo, tighten a comment, change a throw type, fix an `#if NET` branch, etc.) that the fixer agent can make mechanically without external information.
- `[external-action]` — the finding requires something outside this codebase: confirming an issue / ticket exists, verifying CI matrix configuration, asking the user to confirm intent, auditing call sites in production code outside the diff, "worth a quick conformance test asserting…" suggestions where the test would require Atlas infrastructure you don't have, updating commit-message / PR-description wording, or any other action the fixer agent cannot perform without leaving the repo.
...
- `[blocking]` — should land an `escalate` verdict and stop the merge. Public-API breaks; persisted-document-shape changes (the `_id` swap policy, vector-property serialization, Guid representation); MEVD-mandated override signature drift; multi-target build break; security regression.
- `[substantive]` — real concern the fixer should address: wrong behavior, missing test for a non-trivial code path, broken `#if NET` branch, layering violation, lost cancellation propagation, missing AOT/trim attribute on a reflection-using public method, fixture-discipline regression in `MongoTestStore`.
- `[nit]` — mechanical cosmetic: unused import, comment/message typo, misnamed local, missing trailing newline, wording fix-up in a skip / NotSupported message. Does not affect behavior; safe to defer indefinitely.

When unsure between substantive and nit, prefer `substantive` (conservative — it just means the fixer will act on it). When unsure between blocking and substantive, prefer `blocking` if the change can corrupt stored data, break the public API contract, or pass on one target framework while failing on another.
...
Group by reviewer (`api-stability-reviewer`, `mevd-conformance-reviewer`, `security-reviewer`). For each: verdict + findings as bullets. Always show this section, even if all three approved (in which case list each as `<reviewer> — clean`).
...
- **External PR mode**: stem is `review<number>` (e.g. `review12`). Always saved.
- **External clone mode**: stem is `review-<sanitized-head-label>`, where the head label is the branch name or short SHA captured in step 1 with every character that is not `[A-Za-z0-9._-]` replaced by `-` (e.g. branch `feature/foo` → `review-feature-foo`). Always saved.
...
5. Tell the user the filename at the end of your response (one line, e.g. `Saved to review12a.md` or `Saved to review-feature-x.md`).
...
4. Respect the codebase guidance. The parent agent's working directory has up-to-date `AGENTS.md` files — read them as needed via relative paths. In particular: `<Nullable>enable</Nullable>` is on — annotate new types accordingly; library code uses `ConfigureAwait(false)`; `CancellationToken` flows through unchanged; multi-target code uses `#if NET` define constants — a fix that compiles on one target framework must compile on all four (`net10.0`, `net8.0`, `netstandard2.1`, `net472`); reflection-driven public methods carry `[RequiresUnreferencedCode]` + `[RequiresDynamicCode]`. The diff-repo at <diff-repo> may have *older* `AGENTS.md` files — prefer the up-to-date ones from the parent agent's cwd when they conflict.
...
- A diff against a feature branch's own base is the typical use; pass an explicit `<base-ref>` for non-`main` cases (e.g. `/review-mevd-provider release/1.0`). Pass a second positional arg to cap the end of the range (e.g. `/review-mevd-provider HEAD~5 HEAD~1`).
...
- The loop has two convergence guards beyond the max-iterations cap. **`[external-action]` findings don't block convergence**: reviewers tag any finding that requires looking up an issue, verifying CI config, auditing call sites outside the diff, or otherwise leaving the codebase, and those tags exclude the finding from the clean check. The user still sees them — they're listed in every iteration's report and in the closing summary — but the fixer can't address them so they shouldn't pin the loop open. **A no-op fixer ends the loop**: if the fixer judges that nothing in the current report is mechanically actionable, the next iteration would just be re-rolling LLM verdicts on the same tree, so the loop stops with outcome `no actionable findings remaining` and surfaces the outstanding `[external-action]` items.
- Test-running by reviewers is opportunistic. **Unit tests** (`dotnet test test/MongoDB.UnitTests`) are fast and the right tool when a reviewer wants to settle a behavior question without Atlas. **Conformance tests** (`dotnet test test/MongoDB.ConformanceTests`) require Docker (for Atlas Local) or a configured `MongoDB__ConnectionURL` and are slow — don't expect reviewers to run them in a pass; tag conformance-test suggestions as `[external-action]` instead.
